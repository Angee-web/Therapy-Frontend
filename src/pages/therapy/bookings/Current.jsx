...
const Current = () => {
  ...
  const goBack = () => {
    if (currentStep === 1) {
      navigate(`/therapy/therapist-details/${therapist._id}`); // Navigate back in history
      setTimeout(() => {
  ...
  const handleContinue = () => {
    // Validate and prepare scheduling data
    if (isSchedulingComplete && schedulingData) {
      // Fetch therapistId, therapistName, and userId from localStorage if not already in schedulingData
      const therapistId =
        schedulingData.therapistId ||
        localStorage.getItem("selectedTherapistId");
      const therapistName =
        schedulingData.therapistName ||
        localStorage.getItem("selectedTherapistName");
      const userId = schedulingData.userId || localStorage.getItem("userId");

      // Validate that all necessary fields are present
      if (!therapistId || !userId || !therapistName) {
        console.error(
          "Missing therapistId, therapistName, or userId in scheduling data."
        );
        toast({
          title: "Error",
          description:
      ...
      // Add the missing fields to schedulingData
      const completeSchedulingData = {
        ...schedulingData,
        therapistId,
        therapistName,
        userId,
      ...
      // Dispatch the scheduling data
      dispatch(postSchedule(completeSchedulingData))
        .unwrap()
        .then(() => {
          toast({
            title: "Success",
    ...
    // Validate and prepare questionnaire data
    if (isQuestionnaireComplete && questionnaireData) {
      // Fetch therapistId, therapistName, and userId from localStorage if not already in questionnaireData
      const therapistId =
        questionnaireData.therapistId ||
        localStorage.getItem("selectedTherapistId");
      const therapistName =
        questionnaireData.therapistName ||
        localStorage.getItem("selectedTherapistName");
      const userId = questionnaireData.userId || localStorage.getItem("userId");

      // Validate that all necessary fields are present
      if (!therapistId || !userId || !therapistName) {
        console.error(
          "Missing therapistId, therapistName, or userId in questionnaire data."
        );
        toast({
          title: "Error",
          description:
      ...
      // Add the missing fields to questionnaireData
      ...
      // Dispatch the questionnaire data
      dispatch(postQuestionnaire(completeQuestionnaireData))
        .unwrap()
        .then(() => {
          toast({
            title: "Success",
  ...
  const selectedPackages = JSON.parse(localStorage.getItem("selectedPackages"));
     // Redirect to confirm details page or handle your logic here
     navigate("/therapy/confirm-details");
    ...
    <div className="flex flex-col items-center justify-center bg-[#F5F5DC] pb-24 ">
      <div className="pt-8 mx-12">
        ...
        <div className="flex items-center justify-center mt-4 ">
          <button
            onClick={currentStep === 3 ? handleBuyNow : handleContinue}
