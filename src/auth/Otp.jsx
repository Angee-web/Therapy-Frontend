...
import { forgotPassword, verifyOtp } from "@/store/auth-slice.js";
...
const Otp = () => {
  ...
  // Prevent redirection to /auth/login
  useEffect(() => {
    if (!email) {
      toast({
        title: "Error",
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(verifyOtp({ email, otp: otpCode }))
      .unwrap()
      .then((res) => {
        // console.log("OTP Verification Response:", res); // Debugging
        ...
        setCurrentStep(3);
        // console.log("Navigating to /auth/reset-password"); // Debugging
        navigate("/auth/reset-password");
      })
      .catch((error) => {
        // console.error("OTP Verification Error:", error); // Debugging
        toast({
          title: "Error",
    ...
    <div className="bg-shadowTherapy bg-cover bg-center min-h-screen flex justify-center items-center px-4 sm:px-8">
      <div className="shadow-lg backdrop-blur bg-black/30 rounded-2xl w-full max-w-lg lg:max-w-2xl flex flex-col justify-center items-center py-8 px-6 sm:py-10 sm:px-12">
        <div className="text-center text-white flex flex-col items-center w-full">
          {/* Back to Home */}
          <div className="self-start mb-4" onClick={handleNavigateBack}>
            <Link className="flex items-center gap-1 text-xs sm:text-sm">
